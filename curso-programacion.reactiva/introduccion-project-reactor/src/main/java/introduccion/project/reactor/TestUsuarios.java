...
public class TestUsuarios {
	...
	public static void main(String[] args) {
		Flux<String> nombres=Flux.just("Cristian Ramirez","Mery Ramirez","Biaggio cjava","Julen Oliva","Adrian Ramirez","Steven Oliva","Makol Ramirez");
		Flux<Usuario> usuarios=nombres.map(nombre->new Usuario(nombre.split(" ")[0].toUpperCase(),nombre.split(" ")[1].toUpperCase()))
				.filter(usuario->usuario.getApellido().equalsIgnoreCase("cjava"))
				.doOnNext(usuario->{
					if(usuario==null) {
						throw new RuntimeException("Los nombres no puede estar vacios");
					}
